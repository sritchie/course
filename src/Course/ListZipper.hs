...
findLeft f z =
  let check x d = if f x then IsZ z else d
      inner (ListZipper Nil x _) = check x IsNotZ
      inner (ListZipper (l:.ls) x r) =
        check x (inner (ListZipper ls l (x :. r)))
  in inner z

-- This answer matches Tony Morris's definition, but not the
-- properties above.
--
-- findLeft :: (a -> Bool) -> ListZipper a -> MaybeListZipper a
-- findLeft _ (ListZipper Nil _ _) = IsNotZ
-- findLeft f (ListZipper (l:.ls) x rs) =
--   let z = ListZipper ls l (x :. rs)
--   in if f l then IsZ z else findLeft f z
...
findRight f z =
  let check x d = if f x then IsZ z else d
      inner (ListZipper _ x Nil) = check x IsNotZ
      inner (ListZipper ls x (r:.rs)) =
        check x (inner (ListZipper (x:.ls) r rs))
  in inner z
...
moveLeftN' x z
  | x == 0    = Right z
  | x < 0     = moveRightN' (negate x) z
  | otherwise = case moveLeft z of
                 IsNotZ -> Left 0
                 IsZ z' -> case moveLeftN' (pred x) z' of
                   Left x' -> Left (succ x')
                   e -> e
...
moveRightN' x z
  | x == 0    = Right z
  | x < 0     = moveLeftN' (negate x) z
  | otherwise = case moveRight z of
                 IsNotZ -> Left 0
                 IsZ z' -> case moveRightN' (pred x) z' of
                   Left x' -> Left (succ x')
                   e -> e
...
nth x _ | x < 0 = IsNotZ
nth x z@(ListZipper l _ _) =
  case moveLeftN' (length l - x) z of
   Left _ -> IsNotZ
   Right z' -> IsZ z'
...
-- prop> rights (end lz) == Nil
  ...
  P.either (const z) id (moveRightN' (length r) z)
...
start z@(ListZipper l _ _) =
  P.either (const z) id (moveLeftN' (length l) z)
...
instance Apply ListZipper where
  ListZipper fl fx fr <*> ListZipper l x r =
    ListZipper (fzip fl l) (fx x) (fzip fr r)
    where fzip = zipWith ($)
...
instance Extend ListZipper where
  f <<= z =
    let sideList moveFn z' = case moveFn z' of
          IsNotZ -> Nil
          IsZ z'' -> f z'' :. sideList moveFn z''
    in ListZipper
       (sideList moveLeft z)
       (f z)
       (sideList moveRight z)
...
-- Empty
--
-- We have to reverse the ls first to make sure we go through in
-- proper order, since that first business is backwards.
